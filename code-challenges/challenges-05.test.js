...
const templateWithJQuery = () => {
  starWarsPeople.forEach(element => {
    ...
    cloneTemp.find('h2').text(element.name);
    cloneTemp.find('h3').text(element.height);
    cloneTemp.find('p').text(element.eye_color);
    cloneTemp.removeAttr('id');
    cloneTemp.appendTo('main');
  });
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const listFoods = (recipe) => {
  ...
  grocerylist.forEach((element) => {
    let SpaceOne = element.indexOf(' ');
    let SpaceTwo = element.indexOf(' ', SpaceOne + 1);
    result.push(element.slice(SpaceTwo + 1));
  });
  return result;
};
