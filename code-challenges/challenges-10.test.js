...
/* ------------------------------------------------------------------------------------------------
CHALLENGE 1 - Review
Build a simple express server. Connect a '/hello' route that sends a greeting of your  choice.
 Connect a '/aboutme' route that sends a short bio about you to the front-end. Finally, 
connect a '/favoritefoods' route that sends an array to the front-end of your favorite foods.
...
const createServer = () => {
  ...
  app.get('/aboutme',(request,response)=>{
    response.send('My name is Hidaya I  have 21 years old I love food more than anything ');
})


app.get('/favoritefoods',(request,response)=>{
    response.send('all thing we can eat it , i love it ');
})
...
const count = (target, input) => {
  // Solution code here...
  let sum =input.reduce( (accumulator, val)=>{
    let x= val.filter(item=>{
      return item===target;
    })
...
const totalSum = (input) => {
  ...
  return input.reduce((acc, val)=>{
    let supsum =val.reduce((acc,val)=>{ return acc+val; });
     return acc+supsum;
  },0)
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let map_arr=input.map(items=>{
    let filterArry=items.filter(item=>{
        return (typeof (item) === 'number'&& (item %5)===0)
    })
    let power=[];
    for(let i=0; i<filterArry.length;i++)
    {
      power.push(Math.pow(2,filterArry[i]));
    }
    return power;
  })
  return map_arr;
